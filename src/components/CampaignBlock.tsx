  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (typeof window !== 'undefined' && (window as any).grecaptcha) {
      try {
        ...
        setErrorObj(err.message || "Ocurrió un error inesperado al validar la seguridad.");
      } finally {
        setIsSubmitting(false);
      }
    } else {
      setErrorObj("Google reCAPTCHA no pudo cargar correctamente, verifica tu conexión.");
      setIsSubmitting(false);
    }
  };
            ...
            {!isSuccess ? (
              <form onSubmit={handleSubmit} className="flex flex-col gap-4">
                ...
                <div className="flex items-start gap-3 mt-1">
                  <input
                    type="checkbox"
                    id="campaignAcceptTerms"
                    name="acceptTerms"
                    required
                    className="mt-0.5 w-4 h-4 text-primary bg-surface border-outline-variant/30 rounded focus:ring-primary focus:ring-2 accent-primary transition-all cursor-pointer flex-shrink-0"
                  />
                  <label htmlFor="campaignAcceptTerms" className="text-[10px] md:text-xs text-secondary leading-relaxed select-none cursor-pointer">
                    Al enviar tus datos, aceptas nuestra <a href="/politica-de-privacidad/" className="text-primary hover:underline font-bold transition-colors">Política de Privacidad</a>.
                  </label>
                </div>
                ...
                <button
                  ...
                  className="bg-primary text-black mt-2 w-full py-3 rounded-xl text-sm font-headline font-bold hover:shadow-lg transition-all disabled:opacity-70 disabled:cursor-not-allowed flex justify-center items-center gap-2"
                >
                  {isSubmitting ? (
                ...
                <p className="text-[10px] text-secondary/60 text-center mt-2">
                  Este sitio está protegido por reCAPTCHA Enterprise y aplican su <a href="https://policies.google.com/privacy" className="underline hover:text-secondary/80 transition-colors" rel="noreferrer" target="_blank">Política de Privacidad</a> y <a href="https://policies.google.com/terms" className="underline hover:text-secondary/80 transition-colors" rel="noreferrer" target="_blank">Términos de Servicio</a>.
                </p>
              </form>
            ) : (
              <div className="flex flex-col items-center text-center p-4 gap-4">
                ...
                <div>
                  ...
                  <p className="text-sm text-secondary">
                    Ya puedes guardar tu copia de Ergo-logia.
                  </p>
                </div>
